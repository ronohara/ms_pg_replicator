...
logger = logging.getLogger('ms_replicator')
...
def escape_sqlserver_string(value):
    """Escape a string for safe concatenation into SQL Server SQL (without quotes)"""
    if value is None:
        return 'NULL'
...
def convert_dao_value_to_sqlserver_literal(value):
    """Convert DAO value to SQL Server literal string for SQL concatenation"""
    if value is None:
        return 'NULL'
    elif isinstance(value, bool):
        return '1' if value else '0'  # SQL Server BIT uses 1/0
    elif isinstance(value, str):
        return f"'{escape_sqlserver_string(value)}'"
        ...
        return f"'{escape_sqlserver_string(str(value))}'"
...
class ReplicationManager:
    ...
    def validate_configuration(self):
        ...
        ss_params = self.parameters.get('sqlserver', {})
        required_ss = ['host', 'port', 'database', 'user', 'password']
        missing_ss = [p for p in required_ss if not ss_params.get(p)]
        
        if missing_ss:
            error_msg = f"Missing SQL Server parameters: {', '.join(missing_ss)}"
            logger.error(error_msg)
            raise ValueError(error_msg)
        
        try:
            ss_params['port'] = int(ss_params['port'])
        except (ValueError, TypeError):
            error_msg = f"SQL Server port must be an integer, got: {ss_params.get('port')}"
            logger.error(error_msg)
            raise ValueError(error_msg)
    ...
    def get_sqlserver_row_count(self, safe_table_name):
        """Get row count from SQL Server for a table."""
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - get_sqlserver_row_count called for {safe_table_name}")
    ...
    def validate_table_data(self, table_name, safe_table_name):
        ...
        try:
            ...
            if source_count == target_count:
                msg = f"✓ VALIDATED: {table_name} (Access: {source_count}, SQL Server: {target_count})"
                logger.info(msg)
                print(msg)
                return True
            else:
                msg = f"✗ VALIDATION FAILED: {table_name} - Access: {source_count}, SQL Server: {target_count}, Difference: {source_count - target_count}"
                logger.warning(msg)
                print(msg)
                return False
    ...
    def print_validation_summary(self):
        ...
        sync_was_performed = any('ss_count_before' in result for result in self.validation_results.values())
        ...
        if sync_was_performed:
            ...
            summary_lines.append("-" * 110)
            summary_lines.append(f"{'Table Name':<30} {'Access':>10} {'SS (Before)':>12} {'SS (After)':>11} {'Deleted':>10} {'Status':>8} {'Synced':>6}")
            summary_lines.append("-" * 110)
            
            for table_name, result in self.validation_results.items():
                source_count = result.get('source_count', 'N/A')
                # Use fresh count for SS After
                ss_after = fresh_counts.get(table_name, result.get('target_count', 'N/A'))
                deleted_count = result.get('deleted', 0)
                was_synced = 'ss_count_before' in result
                
                if was_synced:
                    ss_before = result.get('ss_count_before', 'N/A')
                    expected_deleted = ss_before - ss_after if isinstance(ss_before, int) and isinstance(ss_after, int) else 0
                    status = "✓ PASS" if (source_count == ss_after and deleted_count == expected_deleted) else "✗ FAIL"
                    synced_flag = "Yes"
                else:
                    ss_before = 'N/A'
                    status = "✓ PASS" if source_count == ss_after else "✗ FAIL"
                    synced_flag = "No"
                
                display_name = table_name[:29] if len(table_name) > 29 else table_name
                deleted_display = str(deleted_count) if deleted_count > 0 else ""
                
                summary_lines.append(f"{display_name:<30} {source_count:>10} {ss_before:>12} {ss_after:>11} {deleted_display:>10} {status:>8} {synced_flag:>6}")
            ...
            inconsistent_tables = []
            for table_name, result in self.validation_results.items():
                # Only check tables that were processed by sync_deleted (have ss_count_before)
                if 'ss_count_before' not in result:
                    continue  # Skip tables that were never synced
                
                source_count = result.get('source_count', 0)
                ss_after = fresh_counts.get(table_name, result.get('target_count', 0))
                ss_before = result.get('ss_count_before', 0)
                deleted = result.get('deleted', 0)
                
                if source_count != ss_after:
                    inconsistent_tables.append((table_name, 'count_mismatch', source_count, ss_after))
                elif deleted != (ss_before - ss_after):
                    inconsistent_tables.append((table_name, 'deleted_mismatch', deleted, ss_before - ss_after))
            ...
            if inconsistent_tables:
                ...
                for table_name, issue, actual, expected in inconsistent_tables:
                    if issue == 'count_mismatch':
                        summary_lines.append(f"  - {table_name}: SQL Server count ({actual}) does not match Access ({expected}) after sync")
                    else:
            ...
            summary_lines.append(f"{'Table Name':<30} {'Access':>10} {'SQL Server':>10} {'Status':>8}")
            summary_lines.append("-" * 70)
        ...
        if total_deleted > 0:
            summary_lines.append(f"Total rows deleted from SQL Server: {total_deleted}")
    ...
    def insert_row(self, safe_table_name, columns, row_data, conflict_clause=None):
        """Insert a single row into SQL Server"""
        if self.trace:
            frame = inspect.currentframe()
        ...
        row_values = [convert_dao_value_to_sqlserver_literal(val) for val in row_data]
        ...
        try:
            self.ss_cursor.execute(insert_sql)
            self.ss_conn.commit()
            return 1
        except Exception as e:
    ...
    def table_exists_in_sqlserver(self, safe_table_name):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - table_exists_in_sqlserver called")
    ...
    def row_exists_in_sqlserver(self, safe_table_name, key_columns, key_values):
        """Check if a row exists in SQL Server by its key columns."""
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - row_exists_in_sqlserver called")
    ...
    def copy_table(self, table_info):
        ...
        # Get target row count from SQL Server
        count_sql = f"SELECT COUNT(*) FROM {safe_table_name}"
        result = self.ss_sql_execute(count_sql, fetch_one=True)
        ...
        if self.nonvolatile and is_nonvolatile:
            if source_row_count == target_row_count:
                if self.slow:
                    ...
                    msg = f"Table {table_name} (non-volatile): row counts match (Access: {source_row_count}, SQL Server: {target_row_count}) - SLOW MODE: copying anyway (optimization disabled for debugging)"
                    logger.info(msg)
                    print(msg)
                    # Continue to normal copy below (do NOT return)
                else:
                    # Normal optimization - skip copy
                    msg = f"Table {table_name} (non-volatile): row counts match (Access: {source_row_count}, SQL Server: {target_row_count}) - skipping copy (--nonvolatile enabled)"
                    logger.info(msg)
                    print(msg)
                ...
                msg = f"Table {table_name} (non-volatile): row counts differ (Access: {source_row_count}, SQL Server: {target_row_count}) - copying despite --nonvolatile"
                logger.info(msg)
                if self.verbose:
                    print(msg)
        ...
        if not has_unique_constraint:
            ...
            if target_row_count == 0:
                ...
                msg = f"Table {table_name} (append-only): up to date (Access: {source_row_count}, SQL Server: {target_row_count})"
                logger.info(msg)
                print(msg)
                self.validation_results[table_name] = {
                    'source_count': source_row_count,
        ...
        # SQL Server does not have ON CONFLICT - use MERGE for UPSERT
        ...
        if has_unique_constraint:
            ...
            if conflict_columns:
                # Build MERGE statement for SQL Server UPSERT
                update_set = ', '.join([f"target.{col} = source.{col}" for col in [col['name'] for col in table_info['columns']]])
                conflict_clause = "MERGE"  # Flag to use MERGE instead of ON CONFLICT
        ...
        while not recordset.EOF:
            ...
            if valid_columns:
                if conflict_clause == "MERGE" and has_unique_constraint:
                    # Use MERGE for SQL Server UPSERT
                    inserted, result_type, details = self.merge_row(
                        safe_table_name, valid_columns, row_data, conflict_columns
                    )
                    if result_type == 'DUPLICATE_EXISTS':
                    ...
                    inserted = self.insert_row(safe_table_name, valid_columns, row_data, None)
                    rows_inserted += inserted
                    violation_count += (1 - inserted)
    ...
    def merge_row(self, safe_table_name, columns, row_data, key_columns):
        """Insert or update a row using SQL Server MERGE statement with NULL key handling.
        Returns: (inserted, result_type, details)
        result_type: 'SUCCESS', 'DUPLICATE_EXISTS', 'DUPLICATE_REJECTED', 'FK_VIOLATION', 'ERROR'
        """
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - merge_row called with columns={columns}")
        
        if not row_data:
            return 0, 'ERROR', 'No data'
        
        # Check if any key column value is NULL (MERGE doesn't handle NULLs well)
        key_data = []
        for key_col in key_columns:
            try:
                idx = columns.index(key_col)
                key_data.append(row_data[idx])
            except ValueError:
                # Key column not found in columns - this shouldn't happen
                key_data.append(None)
        
        has_null_key = any(v is None for v in key_data)
        
        # Build key string for logging
        key_str = ', '.join([f"{k}={v}" for k, v in zip(key_columns, key_data)]) if key_data else 'unknown'
        
        if has_null_key:
            # For rows with NULL keys, use simple INSERT (assume no conflict possible with NULL keys)
            if self.debug:
                logger.debug(f"Row has NULL key value, using INSERT instead of MERGE for {safe_table_name}: key={key_str}")
            ...
            try:
                ...
                self.ss_conn.rollback()
                if '2601' in str(e) or 'duplicate key' in str(e).lower() or 'violation' in str(e).lower():
                    # Check if row actually exists
                    key_values_for_check = [v for v in key_data if v is not None]
                    if key_values_for_check:
                        row_exists = self.row_exists_in_sqlserver(safe_table_name, key_columns, key_values_for_check)
                        if row_exists:
                            if self.debug:
                                logger.debug(f"INSERT failed (duplicate) but row exists for NULL key: {e}")
                            return 1, 'DUPLICATE_EXISTS', key_str
                        else:
                            logger.warning(f"ROW REJECTED (NULL key duplicate): {safe_table_name} - key={key_str}, error={e}")
                            return 0, 'DUPLICATE_REJECTED', key_str
                    else:
                        # Cannot check existence without key values
                        logger.warning(f"ROW REJECTED (NULL key duplicate, no key values): {safe_table_name} - error={e}")
                        return 0, 'DUPLICATE_REJECTED', key_str
                elif 'foreign key' in str(e).lower():
                    logger.warning(f"Foreign key violation in INSERT for {safe_table_name}: key={key_str}, error={e}")
                    return 0, 'FK_VIOLATION', key_str
                else:
                    # Don't log warning for every row with NULL key to avoid spam
                    if self.debug:
            ...
            merge_sql = f"""
                ...
                WHEN NOT MATCHED THEN
                    INSERT ({col_list}) VALUES ({insert_values});
            """
            ...
            try:
                ...
                self.ss_conn.rollback()
                if '2601' in str(e) or 'duplicate key' in str(e).lower():
                    # Extract key values for existence check
                    key_values_for_check = []
                    for key_col in key_columns:
                        try:
                            idx = columns.index(key_col)
                            key_values_for_check.append(row_data[idx])
                        except ValueError:
                            key_values_for_check.append(None)
                    
                    # Check if row actually exists in target
                    row_exists = False
                    if not any(v is None for v in key_values_for_check):
                        row_exists = self.row_exists_in_sqlserver(safe_table_name, key_columns, key_values_for_check)
                    
                    if row_exists:
                        if self.debug:
                            logger.debug(f"Duplicate key error but row exists in {safe_table_name}: key={key_str}")
                        return 1, 'DUPLICATE_EXISTS', key_str
                    else:
                        # Row does not exist - this is a real problem (likely NULL key issue)
                        logger.warning(f"ROW REJECTED: {safe_table_name} - duplicate key error but row does not exist. Key values: {key_str}, Error: {e}")
                        return 0, 'DUPLICATE_REJECTED', key_str
                elif 'foreign key' in str(e).lower():
                    logger.warning(f"Foreign key violation in MERGE for {safe_table_name}: key={key_str}, error={e}")
                    return 0, 'FK_VIOLATION', key_str
                else:
                    logger.warning(f"Failed to merge row in {safe_table_name}: key={key_str}, columns={columns}, error={e}")
                    return 0, 'ERROR', key_str
    ...
    def sanitise_token_for_sqlserver(self, token):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - sanitise_token_for_sqlserver called with params: token={token}")
        ...
        # Remove PostgreSQL double quotes if present
        if token.startswith('"') and token.endswith('"'):
            token = token[1:-1]
    ...
    def sanitise_keyname_for_sqlserver(self, keyname):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - sanitise_keyname_for_sqlserver called with params: keyname={keyname}")
        ...
        if keyname and keyname[0].isdigit():
            keyname = '_' + keyname
        # SQL Server has 128 character limit for identifier names
        if len(keyname) > 128:
            keyname = keyname[:128]
        return keyname
    ...
    def normalise_index_name(self, table_name, index_name):
        ...
        if index_name_clean.startswith(prefix):
            return self.sanitise_keyname_for_sqlserver(index_name)
        combined = f"{table_name_clean}_{index_name_clean}"
        normalised = self.sanitise_keyname_for_sqlserver(combined)
        MAX_IDENTIFIER_LENGTH = 128  # SQL Server limit
        if len(normalised) > MAX_IDENTIFIER_LENGTH:
            truncated = normalised
            while len(truncated) > MAX_IDENTIFIER_LENGTH and len(truncated) > 0:
    ...
    def convert_ms_datatype_sqlserver(self, dao_type, field, column_name):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - convert_ms_datatype_sqlserver called with params: dao_type={dao_type}, column_name={column_name}")
        ...
        type_map = {
                    ...
                    3: 'INT',                    # dbInteger
                    4: 'BIGINT',                 # dbLong
                    5: 'DECIMAL(19,4)',          # dbCurrency
                    6: 'FLOAT',                  # dbSingle
                    7: 'FLOAT',                  # dbDouble
                    8: 'DATETIME',               # dbDate
                    9: 'VARBINARY(MAX)',         # dbBinary
                    10: None,                    # dbText - handled separately
                    11: 'VARBINARY(MAX)',        # dbLongBinary
                    12: 'NVARCHAR(MAX)',         # dbMemo
                    15: 'UNIQUEIDENTIFIER',      # dbGUID
                    16: 'BIGINT',                # dbBigInt
                    20: 'DECIMAL(19,4)',         # dbDecimal
                    21: 'BIGINT',                # dbDBTime
                    22: 'TIME',                  # dbTime
                    23: 'DATETIME',              # dbTimeStamp
                }
        
        # Handle dbText (dao_type 10) specially - need to choose a length that can be indexed
        if dao_type == 10:
            # Check if field has a defined size
            try:
                field_size = field.Size
                if field_size and field_size > 0 and field_size <= 4000:
                    # SQL Server index key columns max 900 bytes for non-clustered
                    # NVARCHAR uses 2 bytes per character, so limit to 450 for safety
                    if field_size <= 450:
                        logger.debug(f"Column {column_name} using NVARCHAR({field_size}) from field.Size")
                        return f'NVARCHAR({field_size})'
                    else:
                        logger.warning(f"Column {column_name} has size {field_size} - using NVARCHAR(450) for index compatibility")
                        return 'NVARCHAR(450)'
                else:
                    # No size defined or size > 4000, use NVARCHAR(255) as safe default
                    logger.debug(f"Column {column_name} using NVARCHAR(255) (default)")
                    return 'NVARCHAR(255)'
            except:
                # If cannot get size, use safe default
                logger.debug(f"Column {column_name} using NVARCHAR(255) (default, could not read field.Size)")
                return 'NVARCHAR(255)'
        
        ss_datatype = type_map.get(dao_type, 'NVARCHAR(255)')
        return ss_datatype
    ...
    def open_sqlserver_connection(self):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - open_sqlserver_connection called")
        
        ss_params = self.parameters.get('sqlserver', {})
        required = ['host', 'port', 'database', 'user', 'password']
        for param in required:
            if param not in ss_params or ss_params[param] is None or ss_params[param] == '':
                logger.error(f"Missing SQL Server parameter: {param}")
                return False
        
        database_name = ss_params.get('database', '')
        if not database_name:
            logger.error("Missing SQL Server database name in configuration")
            return False
        
        try:
            self.ss_conn = pymssql.connect(
                server=ss_params['host'],
                port=int(ss_params['port']),
                database=database_name,
                user=ss_params['user'],
                password=ss_params['password'],
                autocommit=False
            )
            self.ss_cursor = self.ss_conn.cursor()
            logger.info(f"Connected to SQL Server database: {database_name}")
            if self.verbose:
            ...
            logger.error(f"4. Database '{database_name}' exists")
            raise
    
    def open_sqlserver_connection_master(self):
        """Connect to SQL Server using 'master' database for testing purposes only."""
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - open_sqlserver_connection_master called")
        ...
        try:
            ...
            self.ss_cursor = self.ss_conn.cursor()
            logger.info("Connected to SQL Server master database for testing")
            if self.verbose:
                print("Connected to SQL Server master database for testing")
            return True
        except pymssql.OperationalError as e:
            logger.error(f"SQL Server connection failed: {e}")
            raise
    ...
    def ss_sql_execute(self, sql, fetch_one=False, fetch_all=False):
        if self.trace:
            ...
            logger.info(f"Line {frame.f_lineno} - ss_sql_execute called with sql length: {len(sql)} chars, fetch_one={fetch_one}, fetch_all={fetch_all}")
            if self.debug:
        ...
        try:
            ...
            if fetch_all:
                return self.ss_cursor.fetchall()
            return None
        except Exception as e:
    ...
    def load_table(self, table_name):
        ...
        for i in range(tdef.Fields.Count):
            ...
            col_info = {
                ...
                'name': safe_name,
                'type': self.convert_ms_datatype_sqlserver(fld.Type, fld, orig_name),
    ...
    # ========================================================================
    # PRIMARY KEY AND UNIQUE CONSTRAINT CHECKS (SQL Server System Views)
    # ========================================================================
    ...
    # ========================================================================
    # NOTE: SQL Server does NOT require the child side of a foreign key to be unique.
    ...
    def _create_unique_constraint_on_base_table(self, base_table, base_columns, fk_name):
        ...
        NOTE: This method should not be called as SQL Server does not require uniqueness
        ...
        logger.warning(f"NOT CREATING UNIQUE constraint on {base_table} - SQL Server does not require uniqueness on the child side of a foreign key. The foreign key will reference the parent's PRIMARY KEY directly.")
        return None
    ...
    def ensure_uniqueness_on_base_table(self, base_table, base_columns, fk_name):
        ...
        unique constraints that cause replication failures.
        """
        if self.trace:
            frame = inspect.currentframe()
    ...
    def adjust_ms_access_schema(self):
        ...
        # Open only DAO connection (no SQL Server needed)
        try:
            self.open_DAO_connection()
    ...
    def sync_deleted_tables(self, tables_to_process):
        """Synchronize deleted records by comparing row counts between Access and SQL Server.
        ...
        if self.slow:
            ...
            for table_info in tables_to_process:
                ...
                ss_count = self.get_sqlserver_row_count(safe_table_name)
                self._sync_deleted_table(table_name, safe_table_name, access_count, ss_count)
            return
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            for table_info in tables_to_process:
                ...
                try:
                    count_sql = f"SELECT COUNT(*) FROM {safe_table_name}"
                    result = self.ss_sql_execute(count_sql, fetch_one=True)
                    ss_count = result[0] if result else 0
                except Exception as e:
                    print(f"  ✗ {table_name}: Failed to get SQL Server row count - {e}")
                    logger.error(f"sync_deleted_tables: Failed to get SQL Server count for {table_name} - {e}")
                    self.scanned_tables.add(table_name)
                    continue
                
                # Diagnostic logging to compare copy phase vs sync phase
                original_validation = self.validation_results.get(table_name, {})
                logger.info(f"sync_deleted_tables: {table_name} - Copy phase: Access={original_validation.get('source_count')}, SQL Server={original_validation.get('target_count')}")
                logger.info(f"sync_deleted_tables: {table_name} - Current check: Access={access_count}, SQL Server={ss_count}")
                
                print(f"  {table_name}: Access={access_count}, SQL Server={ss_count}")
                logger.info(f"sync_deleted_tables: {table_name} Access={access_count}, SQL Server={ss_count}")
                
                # Only process if counts differ
                if access_count != ss_count:
    ...
    # ========================================================================
    # SQL Server version uses OFFSET-FETCH pagination instead of tuple comparison
    
    def _sync_deleted_table(self, table_name, safe_table_name, access_count, ss_count):
        """Delete rows from SQL Server that no longer exist in MS Access.
        Uses PRIMARY KEY or first UNIQUE constraint to identify matching rows.
        Processes in batches using OFFSET-FETCH pagination.
        """
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - _sync_deleted_table called for {table_name}")
        
        print(f"      Syncing {table_name}: SQL Server has {ss_count} rows, Access has {access_count} rows")
        logger.info(f"_sync_deleted_table: Processing {table_name} (Access: {access_count}, SQL Server: {ss_count})")
        
        # Store original ss_count for validation summary
        original_ss_count = ss_count
        ...
        if pk_columns:
            ...
            if unique_indexes:
                ...
                self.validation_results[table_name] = {
                    ...
                    'target_count': ss_count,
                    'ss_count_before': ss_count,
                    'matched': False,
                    'difference': access_count - ss_count,
                    'deleted': 0,
        ...
        if not key_columns:
            print(f"      ⚠ WARNING: {table_name} has no usable key columns - skipping")
            logger.warning(f"_sync_deleted_table: {table_name} has no usable key columns - skipping")
            self.validation_results[table_name] = {
                'source_count': access_count,
                'target_count': ss_count,
                'ss_count_before': ss_count,
        ...
        # Batch processing parameters using OFFSET-FETCH
        batch_size = 1000
        total_deleted = 0
        total_errors = 0
        offset = 0
        ...
        while True:
            ...
            try:
                self.ss_cursor.execute(select_sql)
                batch_rows = self.ss_cursor.fetchall()
            except Exception as e:
                print(f"      ✗ ERROR: Failed to fetch batch at offset {offset} from SQL Server for {table_name}: {e}")
                logger.error(f"_sync_deleted_table: Failed to fetch batch at offset {offset} from {table_name}: {e}")
                break
            ...
            # Build a mapping of column name to index from cursor description
            col_index_map = {}
            for idx, desc in enumerate(self.ss_cursor.description):
                col_index_map[desc[0].lower()] = idx
            
            key_indices = []
            for key_col in key_columns:
                key_col_lower = key_col.lower()
                if key_col_lower in col_index_map:
                    key_indices.append(col_index_map[key_col_lower])
                else:
                    logger.warning(f"_sync_deleted_table: Key column {key_col} not found in result set for {table_name}")
                    key_indices = []
                    break
            ...
            for pg_row in batch_rows:
                # Extract key values
                current_key_values = [pg_row[idx] for idx in key_indices]
                ...
                for idx in key_indices:
                    ...
                    if val is None:
                        ...
                        access_key_values.append(f"'{escape_sqlserver_string(str(val))}'")
            ...
            if current_time - last_progress_time >= progress_interval:
                ...
                if access_count > 0:
                    ...
                    print(f"      Progress: Offset {offset}, Processed approx {rows_processed_approx}/{access_count} rows [{bar}] {percentage:.1f}%{eta_str} - Deleted {total_deleted} so far", end='\r', flush=True)
        ...
        count_sql = f"SELECT COUNT(*) FROM {safe_table_name}"
        result = self.ss_sql_execute(count_sql, fetch_one=True)
        final_ss_count = result[0] if result else 0
        ...
        self.validation_results[table_name] = {
            'source_count': access_count,
            'target_count': final_ss_count,
            'ss_count_before': original_ss_count,
            'matched': access_count == final_ss_count,
            'difference': access_count - final_ss_count,
            'deleted': total_deleted,
            'errors': total_errors
        }
    
    # ========================================================================
    # LIST DATABASE TABLES (DAO ONLY - NO SQL Server)
    # ========================================================================
    ...
    def create_table(self, table_info):
        ...
        table_exists = self.table_exists_in_sqlserver(safe_table_name)
    ...
    # ========================================================================
    # CREATE INDEXES (WITH FILTERED UNIQUE INDEX SUPPORT FOR NULLABLE COLUMNS)
    # ========================================================================
    ...
    def create_all_indexes(self, tables_to_process):
        ...
        for table_info in tables_to_process:
            ...
            for idx in table_info.get('indexes', []):
                    ...
                    for col in table_info['columns']:
                        if col['original_name'] == col_name or \
                           self.normalize_column_name(col['original_name']) == self.normalize_column_name(col_name):
                            ...
                            # Check if column type is incompatible with indexing in SQL Server
                            col_type_upper = col['type'].upper()
                            if 'MAX' in col_type_upper or col_type_upper in ['TEXT', 'NTEXT', 'IMAGE', 'XML']:
                                logger.warning(f"Index {idx['name']} on table {table_info['name']} skipped: column '{col['name']}' has type {col['type']} which cannot be used as an index key column in SQL Server")
                                skip_index = True
                                break
                            # Track nullable columns for filtered unique index
                ...
                try:
                    if idx.get('unique'):
                        # Check if all columns in the unique index are nullable
                        if nullable_columns and len(nullable_columns) == len(idx_columns):
                            # All columns are nullable - use filtered unique index to allow multiple NULLs
                            # This matches Access behavior where unique indexes ignore NULLs
                            where_clause = ' AND '.join([f"{col} IS NOT NULL" for col in idx_columns])
                            sql = f"CREATE UNIQUE INDEX {safe_idx_name} ON {safe_table_name} ({', '.join(idx_columns)}) WHERE {where_clause}"
                            logger.info(f"Creating filtered unique index {idx['name']} on {table_info['name']} (allows multiple NULLs, matches Access behavior)")
                        else:
                            # Some columns are NOT NULL - use standard unique index
                            sql = f"CREATE UNIQUE INDEX {safe_idx_name} ON {safe_table_name} ({', '.join(idx_columns)})"
                            logger.info(f"Creating unique index {idx['name']} on {table_info['name']}")
                    else:
                        sql = f"CREATE INDEX {safe_idx_name} ON {safe_table_name} ({', '.join(idx_columns)})"
                        logger.info(f"Creating index {idx['name']} on {table_info['name']}")
                    
                    self.ss_sql_execute(sql)
                    self.ss_conn.commit()
                except Exception as e:
                    logger.warning(f"Failed to create index {idx['name']} on {table_info['name']}: {e}")
                    self.ss_conn.rollback()
...
"""
    ...
    def replicate_schema(self):
        ...
        ss_params = self.parameters.get('sqlserver', {})
        dbname = ss_params.get('database', '')
        
        # SQL Server - need to connect to master to create/drop database
        admin_params = ss_params.copy()
        admin_params['database'] = 'master'
        admin_conn = pymssql.connect(
            server=admin_params['host'],
...
"""
    ...
    def replicate_tables(self):
        ...
        if self.full_refresh:
            ...
            for table_info in reversed(tables_to_process):
                ...
                # SQL Server doesn't support CASCADE on DROP TABLE
                # Need to drop foreign keys first - handled by dropping in reverse dependency order
                drop_sql = f"DROP TABLE IF EXISTS {safe_name}"
                self.ss_sql_execute(drop_sql)
                self.ss_conn.commit()
    ...
    def test_network_connections(self):
        ...
        try:
            # Use master database for connection test
            self.open_sqlserver_connection_master()
    ...
    def dump_internal_data(self):
        ...
        if self.validation_results:
            for table_name, result in self.validation_results.items():
                ...
                print(f"  {status} {table_name}: Access={source}, SS Before={ss_before}, SS After={target}, Deleted={deleted}{nv_flag}")
        else:
    ...
    def generate_yaml_file(self):
        ...
        sections_order = ['global', 'DAO', 'sqlserver', 'excluded', 'nonvolatile', 'tables', 'primarykeys', 'foreignkeys', 'transformations']
...
def main():
    ...
    parser.add_argument('--sdatabase', help='SQL Server database name')
    parser.add_argument('--suser', help='SQL Server user name')
    parser.add_argument('--spassword', help='SQL Server password')
    ...
    parser.add_argument('--sync-deleted', action='store_true',
                        help='Synchronize deleted records from Access to SQL Server')
    ...
    if args.version:
        print(f"MS Access to SQL Server Replication Tool {version_raw}")
        return
    ...
    with open(args.config, 'r', encoding='utf-8') as f:
        try:
            ...
            # If --adjust-ms-access is set, remove SQL Server section (optional)
            if args.adjust_ms_access and 'sqlserver' in parameters:
                del parameters['sqlserver']
                logger.info("adjust-ms-access: SQL Server configuration ignored")
        except yaml.YAMLError as e:
    ...
    # Only process SQL Server parameters if NOT in adjust-ms-access mode
    if not args.adjust_ms_access:
        if args.shost:
            if 'sqlserver' not in parameters:
                parameters['sqlserver'] = {}
            parameters['sqlserver']['host'] = args.shost
        if args.sport:
            if 'sqlserver' not in parameters:
                parameters['sqlserver'] = {}
            parameters['sqlserver']['port'] = int(args.sport)
        if args.sdatabase:
            if 'sqlserver' not in parameters:
                parameters['sqlserver'] = {}
            parameters['sqlserver']['database'] = args.sdatabase
        if args.suser:
            if 'sqlserver' not in parameters:
                parameters['sqlserver'] = {}
            parameters['sqlserver']['user'] = args.suser
        if args.spassword:
            if 'sqlserver' not in parameters:
                parameters['sqlserver'] = {}
            parameters['sqlserver']['password'] = args.spassword
    ...
    try:
        if manager.list_tables:
            manager.list_database_tables()  # Only DAO connection, no SQL Server
