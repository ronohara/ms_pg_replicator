...
logger = logging.getLogger('my_replicator')
...
def escape_mysql_string(value):
    """Escape a string for safe concatenation into MySQL SQL (without quotes)"""
    if value is None:
        return 'NULL'
...
def convert_dao_value_to_mysql_literal(value):
    """Convert DAO value to MySQL literal string for SQL concatenation"""
    if value is None:
        return 'NULL'
    elif isinstance(value, bool):
        return 'TRUE' if value else 'FALSE'
    elif isinstance(value, str):
        return f"'{escape_mysql_string(value)}'"
...
class ReplicationManager:
    ...
    def exit_program(self, exit_code=0, error_msg=None):
        ...
        if exit_code == 0:
            msg = f"my_replicator.py version {__version__} terminated successfully"
            logger.info(msg)
            if self.verbose:
                print(msg)
        else:
            if error_msg:
                msg = f"my_replicator.py version {__version__} terminated with errors: {error_msg}"
            else:
                msg = f"my_replicator.py version {__version__} terminated with errors (exit code {exit_code})"
            logger.error(msg)
            if self.verbose:
                print(msg)
    ...
    def validate_configuration(self):
        ...
        mysql_params = self.parameters.get('mysql', {})
        required_mysql = ['host', 'port', 'database', 'user', 'password']
        missing_mysql = [p for p in required_mysql if not mysql_params.get(p)]
        
        if missing_mysql:
            error_msg = f"Missing MySQL parameters: {', '.join(missing_mysql)}"
        ...
        try:
            ...
            error_msg = f"MySQL port must be an integer, got: {mysql_params.get('port')}"
            logger.error(error_msg)
            raise ValueError(error_msg)
    ...
    def get_mysql_row_count(self, safe_table_name):
        """Get row count from MySQL for a table."""
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - get_mysql_row_count called for {safe_table_name}")
        
        try:
            count_sql = f"SELECT COUNT(*) FROM {safe_table_name}"
            result = self.mysql_sql_execute(count_sql, fetch_one=True)
    ...
    def validate_table_data(self, table_name, safe_table_name):
        ...
        try:
            ...
            if source_count == target_count:
                msg = f"✓ VALIDATED: {table_name} (Access: {source_count}, MySQL: {target_count})"
                logger.info(msg)
                print(msg)
                return True
            else:
                msg = f"✗ VALIDATION FAILED: {table_name} - Access: {source_count}, MySQL: {target_count}, Difference: {source_count - target_count}"
                logger.warning(msg)
                print(msg)
                return False
    ...
    def print_validation_summary(self):
        ...
        sync_was_performed = any('mysql_count_before' in result for result in self.validation_results.values())
        ...
        if sync_was_performed:
            ...
            summary_lines.append("-" * 110)
            summary_lines.append(f"{'Table Name':<30} {'Access':>10} {'MySQL (Before)':>12} {'MySQL (After)':>11} {'Deleted':>10} {'Status':>8} {'Synced':>6}")
            summary_lines.append("-" * 110)
            
            for table_name, result in self.validation_results.items():
                source_count = result.get('source_count', 'N/A')
                # Use fresh count for MySQL After
                mysql_after = fresh_counts.get(table_name, result.get('target_count', 'N/A'))
                deleted_count = result.get('deleted', 0)
                was_synced = 'mysql_count_before' in result
                
                if was_synced:
                    mysql_before = result.get('mysql_count_before', 'N/A')
                    expected_deleted = mysql_before - mysql_after if isinstance(mysql_before, int) and isinstance(mysql_after, int) else 0
                    status = "✓ PASS" if (source_count == mysql_after and deleted_count == expected_deleted) else "✗ FAIL"
                    synced_flag = "Yes"
                else:
                    mysql_before = 'N/A'
                    status = "✓ PASS" if source_count == mysql_after else "✗ FAIL"
                    synced_flag = "No"
                
                display_name = table_name[:29] if len(table_name) > 29 else table_name
                deleted_display = str(deleted_count) if deleted_count > 0 else ""
                
                summary_lines.append(f"{display_name:<30} {source_count:>10} {mysql_before:>12} {mysql_after:>11} {deleted_display:>10} {status:>8} {synced_flag:>6}")
            ...
            inconsistent_tables = []
            for table_name, result in self.validation_results.items():
                # Only check tables that were processed by sync_deleted (have mysql_count_before)
                if 'mysql_count_before' not in result:
                    continue  # Skip tables that were never synced
                
                source_count = result.get('source_count', 0)
                mysql_after = fresh_counts.get(table_name, result.get('target_count', 0))
                mysql_before = result.get('mysql_count_before', 0)
                deleted = result.get('deleted', 0)
                
                if source_count != mysql_after:
                    inconsistent_tables.append((table_name, 'count_mismatch', source_count, mysql_after))
                elif deleted != (mysql_before - mysql_after):
                    inconsistent_tables.append((table_name, 'deleted_mismatch', deleted, mysql_before - mysql_after))
            ...
            if inconsistent_tables:
                ...
                for table_name, issue, actual, expected in inconsistent_tables:
                    if issue == 'count_mismatch':
                        summary_lines.append(f"  - {table_name}: MySQL count ({actual}) does not match Access ({expected}) after sync")
                    else:
            ...
            summary_lines.append(f"{'Table Name':<30} {'Access':>10} {'MySQL':>10} {'Status':>8}")
            summary_lines.append("-" * 70)
        ...
        if total_deleted > 0:
            summary_lines.append(f"Total rows deleted from MySQL: {total_deleted}")
    ...
    def insert_row(self, safe_table_name, columns, row_data, conflict_clause=None):
        """Insert a single row into MySQL.
        ...
        row_values = [convert_dao_value_to_mysql_literal(val) for val in row_data]
    ...
    def table_exists_in_mysql(self, safe_table_name):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - table_exists_in_mysql called")
        ...
        try:
            ...
            check_sql = f"""
                ...
                AND table_name = '{escape_mysql_string(unquoted_name)}'
            """
            self.mysql_cursor.execute(check_sql)
    ...
    def row_exists_in_mysql(self, safe_table_name, key_columns, key_values):
        """Check if a row exists in MySQL by its key columns."""
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - row_exists_in_mysql called")
        ...
        for col, val in zip(quoted_key_columns, key_values):
            where_parts.append(f"{col} = {convert_dao_value_to_mysql_literal(val)}")
        where_clause = ' AND '.join(where_parts)
        check_sql = f"SELECT COUNT(*) FROM {safe_table_name} WHERE {where_clause}"
        
        try:
            result = self.mysql_sql_execute(check_sql, fetch_one=True)
        ...
        for trans in transformations:
            ...
            for col_entry in columns_list:
                if not isinstance(col_entry, dict):
                    continue
                col_name = col_entry.get('name', '')
                mysql_col_name = self.get_sanitise_function()(col_name)
                if mysql_col_name.lower() != column_name.lower():
                    continue
    ...
    def copy_table(self, table_info):
        ...
        # Get target row count from MySQL
        count_sql = f"SELECT COUNT(*) FROM {safe_table_name}"
        result = self.mysql_sql_execute(count_sql, fetch_one=True)
        ...
        if self.nonvolatile and is_nonvolatile:
            if source_row_count == target_row_count:
                if self.slow:
                    ...
                    msg = f"Table {table_name} (non-volatile): row counts match (Access: {source_row_count}, MySQL: {target_row_count}) - SLOW MODE: copying anyway (optimization disabled for debugging)"
                    logger.info(msg)
                    print(msg)
                    # Continue to normal copy below (do NOT return)
                else:
                    # Normal optimization - skip copy
                    msg = f"Table {table_name} (non-volatile): row counts match (Access: {source_row_count}, MySQL: {target_row_count}) - skipping copy (--nonvolatile enabled)"
                    logger.info(msg)
                    print(msg)
                ...
                msg = f"Table {table_name} (non-volatile): row counts differ (Access: {source_row_count}, MySQL: {target_row_count}) - copying despite --nonvolatile"
                logger.info(msg)
                if self.verbose:
                    print(msg)
        ...
        if not has_unique_constraint:
            ...
            if target_row_count == 0:
                ...
                msg = f"Table {table_name} (append-only): up to date (Access: {source_row_count}, MySQL: {target_row_count})"
                logger.info(msg)
                print(msg)
                self.validation_results[table_name] = {
                    'source_count': source_row_count,
        ...
        if has_unique_constraint:
            ...
            if conflict_columns:
                update_set = ', '.join([f"{col} = VALUES({col})" for col in [col['name'] for col in table_info['columns']]])
                conflict_clause = f"ON DUPLICATE KEY UPDATE {update_set}"
        ...
        while not recordset.EOF:
            ...
            if valid_columns:
                if conflict_clause and has_unique_constraint:
                    # Use ON DUPLICATE KEY UPDATE for MySQL UPSERT
    ...
    def sanitise_token_for_mysql(self, token):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - sanitise_token_for_mysql called with params: token={token}")
    ...
    def sanitise_keyname_for_mysql(self, keyname):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - sanitise_keyname_for_mysql called with params: keyname={keyname}")
        ...
        if keyname and keyname[0].isdigit():
            keyname = '_' + keyname
        # MySQL has 64 character limit for identifier names
        if len(keyname) > 64:
            keyname = keyname[:64]
        return keyname
    ...
    def normalise_index_name(self, table_name, index_name):
        ...
        if index_name_clean.startswith(prefix):
            return self.sanitise_keyname_for_mysql(index_name)
        combined = f"{table_name_clean}_{index_name_clean}"
        normalised = self.sanitise_keyname_for_mysql(combined)
        MAX_IDENTIFIER_LENGTH = 64  # MySQL limit
    ...
    def convert_ms_datatype_mysql(self, dao_type, field, column_name):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - convert_ms_datatype_mysql called with params: dao_type={dao_type}, column_name={column_name}")
        ...
        # Handle dbText (dao_type 10) specially - need to choose a length
        if dao_type == 10:
            # Check if field has a defined size
            try:
                field_size = field.Size
                if field_size and field_size > 0 and field_size <= 65535:
                    # MySQL VARCHAR max is 65535 bytes
                    if field_size <= 255:
                        logger.debug(f"Column {column_name} using VARCHAR({field_size}) from field.Size")
                        return f'VARCHAR({field_size})'
                    else:
                        logger.debug(f"Column {column_name} using TEXT for size {field_size}")
                        return 'TEXT'
                else:
                    # No size defined or size too large, use TEXT
                    logger.debug(f"Column {column_name} using TEXT (default)")
                    return 'TEXT'
            except:
                # If cannot get size, use TEXT
                logger.debug(f"Column {column_name} using TEXT (default, could not read field.Size)")
                return 'TEXT'
        
        mysql_datatype = type_map.get(dao_type, 'TEXT')
    ...
    def open_mysql_connection(self):
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - open_mysql_connection called")
        
        mysql_params = self.parameters.get('mysql', {})
        required = ['host', 'port', 'database', 'user', 'password']
        for param in required:
            if param not in mysql_params or mysql_params[param] is None or mysql_params[param] == '':
                logger.error(f"Missing MySQL parameter: {param}")
                return False
        
        database_name = mysql_params.get('database', '')
        if not database_name:
            logger.error("Missing MySQL database name in configuration")
            return False
    ...
    def open_mysql_connection_master(self):
        """Connect to MySQL without a default database for testing purposes only."""
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - open_mysql_connection_master called")
        ...
        try:
            ...
            self.mysql_cursor = self.mysql_conn.cursor()
            logger.info("Connected to MySQL (no default database) for testing")
            if self.verbose:
                print("Connected to MySQL (no default database) for testing")
            return True
        except mysql.connector.Error as e:
            logger.error(f"MySQL connection failed: {e}")
            raise
    ...
    def close_connections(self):
        ...
        if self.mysql_cursor:
            try:
                ...
                if self.debug:
                    logger.debug(f"Error closing mysql_cursor: {e}")
            self.mysql_cursor = None
        ...
        if self.mysql_conn:
            try:
                ...
                if self.debug:
                    logger.debug(f"Error closing mysql_conn: {e}")
            self.mysql_conn = None
    ...
    def mysql_sql_execute(self, sql, fetch_one=False, fetch_all=False):
        if self.trace:
            frame = inspect.currentframe()
            sql_preview = sql[:500] + "..." if len(sql) > 500 else sql
            logger.info(f"Line {frame.f_lineno} - mysql_sql_execute called with sql length: {len(sql)} chars, fetch_one={fetch_one}, fetch_all={fetch_all}")
            if self.debug:
    ...
    def get_sanitise_function(self):
        ...
        If simple_names is False, returns sanitise_token_for_mysql (quoted, preserves case/spaces using backticks)
        """
        if self.simple_names:
            return self.sanitise_for_sane_view
        else:
            return self.sanitise_token_for_mysql
    ...
    def load_table(self, table_name):
        ...
        for i in range(tdef.Fields.Count):
            ...
            col_info = {
                ...
                'name': safe_name,
                'type': self.convert_ms_datatype_mysql(fld.Type, fld, orig_name),
    ...
    # ========================================================================
    # PRIMARY KEY AND UNIQUE CONSTRAINT CHECKS (MySQL Information Schema)
    # ========================================================================
    ...
    def check_primary_key(self, table_name, columns):
        ...
        # Sanitise the column names to match what MySQL actually has
        ...
        pk_columns_sql = f"""
            ...
            WHERE table_schema = DATABASE()
            AND table_name = '{escape_mysql_string(unquoted_table_name)}'
            AND constraint_name = 'PRIMARY'
            ORDER BY ordinal_position
        """
        pk_columns = self.mysql_sql_execute(pk_columns_sql, fetch_all=True)
    ...
    def check_unique_constraint_only(self, table_name, columns):
        ...
        unique_check_sql = f"""
            ...
            AND constraint_name != 'PRIMARY'
        """
        unique_constraints = self.mysql_sql_execute(unique_check_sql, fetch_all=True)
        
        for constraint_row in unique_constraints or []:
            constraint_name = constraint_row[0]
            # Check if this is a UNIQUE constraint
            constraint_type_sql = f"""
                SELECT constraint_type
                FROM information_schema.table_constraints
                WHERE table_schema = DATABASE()
                AND table_name = '{escape_mysql_string(unquoted_table_name)}'
                AND constraint_name = '{escape_mysql_string(constraint_name)}'
            """
            constraint_type_result = self.mysql_sql_execute(constraint_type_sql, fetch_one=True)
            if constraint_type_result and constraint_type_result[0] == 'UNIQUE':
                ...
                index_cols_sql = f"""
                    ...
                    ORDER BY ordinal_position
                """
                index_columns = self.mysql_sql_execute(index_cols_sql, fetch_all=True)
    ...
    def check_reference_table_has_uniqueness(self, reference_table, reference_columns):
        ...
        pk_sql = f"""
            SELECT LOWER(column_name), constraint_name
            FROM information_schema.key_column_usage
            WHERE table_schema = DATABASE()
            AND table_name = '{escape_mysql_string(unquoted_table)}'
            AND constraint_name = 'PRIMARY'
            ORDER BY ordinal_position
        """
        pk_columns = self.mysql_sql_execute(pk_sql, fetch_all=True)
        if pk_columns:
            pk_col_names = [row[0] for row in pk_columns]
            pk_constraint_name = 'PRIMARY'
        ...
        unique_sql = f"""
            ...
            AND kcu.table_name = '{escape_mysql_string(unquoted_table)}'
            AND tc.constraint_type = 'UNIQUE'
        """
        unique_constraints = self.mysql_sql_execute(unique_sql, fetch_all=True)
        ...
        for constraint_row in unique_constraints or []:
            ...
            index_col_names = [row[0] for row in index_columns] if index_columns else []
            if set(columns_lower) == set(index_col_names):
                logger.info(f"Reference table {reference_table} has UNIQUE constraint {constraint_name} on {reference_columns}")
                return (True, 'UNIQUE', constraint_name)
    ...
    # ========================================================================
    # NOTE: MySQL does NOT require the child side of a foreign key to be unique.
    ...
    def _create_unique_constraint_on_base_table(self, base_table, base_columns, fk_name):
        ...
        NOTE: This method should not be called as MySQL does not require uniqueness
        ...
        logger.warning(f"NOT CREATING UNIQUE constraint on {base_table} - MySQL does not require uniqueness on the child side of a foreign key. The foreign key will reference the parent's PRIMARY KEY directly.")
        return None
    ...
    def _create_primary_key_on_base_table(self, base_table, base_columns, fk_name):
        ...
        constraint_name = self.sanitise_keyname_for_mysql(f"pk_{fk_name}")
        
        # Check if a PRIMARY KEY constraint already exists with this name
        check_sql = f"""
            SELECT 1 FROM information_schema.table_constraints
            WHERE table_schema = DATABASE()
            AND table_name = '{escape_mysql_string(base_table)}'
            AND constraint_name = '{escape_mysql_string(constraint_name)}'
            AND constraint_type = 'PRIMARY KEY'
        """
        exists = self.mysql_sql_execute(check_sql, fetch_one=True)
        if exists:
        ...
        try:
            self.mysql_sql_execute(create_sql)
            self.mysql_conn.commit()
    ...
    def ensure_uniqueness_on_base_table(self, base_table, base_columns, fk_name):
        ...
        logger.warning(f"Foreign key {fk_name}: MySQL does NOT require uniqueness on child table '{base_table}' columns {base_columns}. Skipping automatic creation of unique constraint.")
        return None
    ...
    def create_internal_replicator_table(self):
        ...
        """
        self.mysql_sql_execute(insert_sql)
        self.mysql_conn.commit()
    ...
    def read_internal_replicator_data(self):
        ...
        try:
            ...
            """
            self.mysql_cursor.execute(check_sql)
            table_exists = self.mysql_cursor.fetchone()[0] > 0
            ...
            column_check_sql = """
                ...
                ORDER BY ordinal_position
            """
            self.mysql_cursor.execute(column_check_sql)
            columns = self.mysql_cursor.fetchall()
            
            expected_columns = [('simplenames', 'tinyint')]
    ...
    def adjust_ms_access_schema(self):
        ...
        # Open only DAO connection (no MySQL needed)
        try:
            self.open_DAO_connection()
    ...
    def sync_deleted_tables(self, tables_to_process):
        """Synchronize deleted records by comparing row counts between Access and MySQL.
        ...
        if self.slow:
            ...
            for table_info in tables_to_process:
                ...
                mysql_count = self.get_mysql_row_count(safe_table_name)
                self._sync_deleted_table(table_name, safe_table_name, access_count, mysql_count)
            return
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            for table_info in tables_to_process:
                ...
                try:
                    ...
                    print(f"  ✗ {table_name}: Failed to get MySQL row count - {e}")
                    logger.error(f"sync_deleted_tables: Failed to get MySQL count for {table_name} - {e}")
                    self.scanned_tables.add(table_name)
                    continue
                
                # Diagnostic logging to compare copy phase vs sync phase
                original_validation = self.validation_results.get(table_name, {})
                logger.info(f"sync_deleted_tables: {table_name} - Copy phase: Access={original_validation.get('source_count')}, MySQL={original_validation.get('target_count')}")
                logger.info(f"sync_deleted_tables: {table_name} - Current check: Access={access_count}, MySQL={mysql_count}")
                
                print(f"  {table_name}: Access={access_count}, MySQL={mysql_count}")
                logger.info(f"sync_deleted_tables: {table_name} Access={access_count}, MySQL={mysql_count}")
    ...
    # ========================================================================
    # SYNC DELETED TABLE (PRIVATE METHOD - BATCH PROCESSING WITH OFFSET PAGINATION)
    # ========================================================================
    # MySQL version uses OFFSET pagination since tuple comparison is not supported
    
    def _sync_deleted_table(self, table_name, safe_table_name, access_count, mysql_count):
        """Delete rows from MySQL that no longer exist in MS Access.
        Uses PRIMARY KEY or first UNIQUE constraint to identify matching rows.
        Processes in batches using OFFSET pagination.
        """
        if self.trace:
            frame = inspect.currentframe()
            logger.info(f"Line {frame.f_lineno} - _sync_deleted_table called for {table_name}")
        
        print(f"      Syncing {table_name}: MySQL has {mysql_count} rows, Access has {access_count} rows")
        logger.info(f"_sync_deleted_table: Processing {table_name} (Access: {access_count}, MySQL: {mysql_count})")
        
        # Store original mysql_count for validation summary
        original_mysql_count = mysql_count
        ...
        # First, try to find PRIMARY KEY using MySQL information_schema
        ...
        if pk_columns:
            ...
            if unique_constraints:
                # Get the first unique constraint's columns
                constraint_name = unique_constraints[0][0]
                idx_cols_sql = f"""
                    SELECT LOWER(column_name)
                    FROM information_schema.key_column_usage
                    WHERE table_schema = DATABASE()
                    ...
                    ORDER BY ordinal_position
                """
                idx_columns = self.mysql_sql_execute(idx_cols_sql, fetch_all=True)
                if idx_columns:
                ...
                self.validation_results[table_name] = {
                    ...
                    'mysql_count_before': mysql_count,
                    'matched': False,
                    'difference': access_count - mysql_count,
                    'deleted': 0,
        ...
        columns_sql = f"""
            ...
            ORDER BY ordinal_position
        """
        all_columns = self.mysql_sql_execute(columns_sql, fetch_all=True)
        ...
        # Batch processing parameters using OFFSET pagination
        batch_size = 1000
        total_deleted = 0
        total_errors = 0
        offset = 0
        ...
        while True:
            # Build SELECT query with OFFSET pagination
            select_sql = f"""
            ...
            try:
                self.mysql_cursor.execute(select_sql)
                batch_rows = self.mysql_cursor.fetchall()
            except Exception as e:
                print(f"      ✗ ERROR: Failed to fetch batch at offset {offset} from MySQL for {table_name}: {e}")
            ...
            col_index_map = {}
            for idx, desc in enumerate(self.mysql_cursor.description):
            ...
            for row in batch_rows:
                # Extract key values
                current_key_values = [row[idx] for idx in key_indices]
                ...
                for idx in key_indices:
                    ...
                    if val is None:
                        ...
                        access_key_values.append(f"'{escape_mysql_string(str(val))}'")
            ...
            if keys_to_delete:
                ...
                for i in range(0, len(keys_to_delete), delete_batch_size):
                    ...
                    for pk_tuple in sub_batch:
                        value_literals = [convert_dao_value_to_mysql_literal(v) for v in pk_tuple]
        ...
        count_sql = f"SELECT COUNT(*) FROM {safe_table_name}"
        result = self.mysql_sql_execute(count_sql, fetch_one=True)
        final_mysql_count = result[0] if result else 0
        ...
        self.validation_results[table_name] = {
            'source_count': access_count,
            'target_count': final_mysql_count,
            'mysql_count_before': original_mysql_count,
            'matched': access_count == final_mysql_count,
            'difference': access_count - final_mysql_count,
            'deleted': total_deleted,
            'errors': total_errors
        }
    
    # ========================================================================
    # LIST DATABASE TABLES (DAO ONLY - NO MySQL)
    # ========================================================================
    ...
    def create_table(self, table_info):
        ...
        table_exists = self.table_exists_in_mysql(safe_table_name)
    ...
    def create_all_indexes(self, tables_to_process):
        ...
        for table_info in tables_to_process:
            safe_table_name = table_info['safe_name']
            # Get the unquoted table name for checking (strip backticks if present)
            unquoted_table_name = safe_table_name.strip('`')
            ...
            for idx in table_info.get('indexes', []):
                ...
                # Check if index already exists in MySQL using the unquoted table name
                check_sql = f"""
                    SELECT 1 FROM information_schema.statistics
                    WHERE table_schema = DATABASE()
                    AND table_name = '{escape_mysql_string(unquoted_table_name)}'
                    AND index_name = '{escape_mysql_string(safe_idx_name)}'
                    LIMIT 1
                """
                exists = self.mysql_sql_execute(check_sql, fetch_one=True)
...
"""
        ...
        safe_fk_name = self.sanitise_keyname_for_mysql(fk_name)
        safe_base_table = sanitise_func(base_table)
        safe_ref_table = sanitise_func(reference_table)
        
        # Check if foreign key already exists in MySQL using the unquoted base table name
        unquoted_base_table = safe_base_table.strip('`')
        check_sql = f"""
            SELECT 1
            FROM information_schema.table_constraints
            WHERE table_schema = DATABASE()
            AND table_name = '{escape_mysql_string(unquoted_base_table)}'
            AND constraint_name = '{escape_mysql_string(safe_fk_name)}'
            AND constraint_type = 'FOREIGN KEY'
        """
        existing = self.mysql_sql_execute(check_sql, fetch_one=True)
        ...
        try:
            self.mysql_sql_execute(alter_sql)
    ...
    def replicate_schema(self):
        ...
        mysql_params = self.parameters.get('mysql', {})
        dbname = mysql_params.get('database', '')
        
        # Connect without default database to create/drop database
        ...
        admin_cursor.execute(f"DROP DATABASE IF EXISTS `{escape_mysql_string(dbname)}`")
        logger.info(f"Dropped database: {dbname}")
        
        admin_cursor.execute(f"CREATE DATABASE `{escape_mysql_string(dbname)}` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci")
    ...
    def create_sane_views(self):
        """Create sane views in MySQL for all tables created by replicate_schema.
        ...
        - Uses CREATE OR REPLACE VIEW (MySQL syntax)
        ...
        for table_name in table_names:
            try:
                ...
                for col in table_info['columns']:
                    ...
                    # Use the sanitised column name (already properly formatted for MySQL)
...
"""
                ...
                # Execute the SQL
                self.mysql_sql_execute(create_view_sql)
                self.mysql_conn.commit()
    ...
    def create_views_only(self):
        """Create sane views on an existing MySQL database based on MS Access tables.
        
        This mode:
        - Connects to both MS Access and MySQL
        ...
        - Assumes the target tables already exist in MySQL
        ...
        for table_name in table_names:
            try:
                ...
                if not self.table_exists_in_mysql(safe_original_table):
                    print(f"  ⊘ {original_table_name}: Table does not exist in MySQL (looking for {safe_original_table}) - skipping")
                    logger.warning(f"create_views_only: Table {original_table_name} not found in MySQL (expected {safe_original_table})")
                    continue
...
"""
    ...
    def test_network_connections(self):
        ...
        try:
            # Connect without default database for testing
            self.open_mysql_connection_master()
    ...
    def dump_internal_data(self):
        ...
        if self.validation_results:
            for table_name, result in self.validation_results.items():
                ...
                print(f"  {status} {table_name}: Access={source}, MySQL Before={mysql_before}, MySQL After={target}, Deleted={deleted}{nv_flag}")
        else:
    ...
    def generate_yaml_file(self):
        ...
        sections_order = ['global', 'DAO', 'mysql', 'excluded', 'nonvolatile', 'tables', 'primarykeys', 'foreignkeys', 'transformations']
...
def main():
    ...
    logger.info(f"my_replicator.py version {__version__} starting")
    
    parser = argparse.ArgumentParser(description='MS Access to MySQL Replication Tool')
    ...
    parser.add_argument('--mhost', help='MySQL server host name or IP')
    parser.add_argument('--mport', help='MySQL server port number')
    parser.add_argument('--mdatabase', help='MySQL database name')
    parser.add_argument('--muser', help='MySQL user name')
    parser.add_argument('--mpassword', help='MySQL password')
    ...
    parser.add_argument('--sync-deleted', action='store_true',
                        help='Synchronize deleted records from Access to MySQL')
    ...
    if args.version:
        print(f"MS Access to MySQL Replication Tool {version_raw}")
        return
    ...
    with open(args.config, 'r', encoding='utf-8') as f:
        try:
            ...
            # If --adjust-ms-access is set, remove MySQL section (optional)
            if args.adjust_ms_access and 'mysql' in parameters:
                del parameters['mysql']
                logger.info("adjust-ms-access: MySQL configuration ignored")
        except yaml.YAMLError as e:
    ...
    # Only process MySQL parameters if NOT in adjust-ms-access mode
    if not args.adjust_ms_access:
        if args.mhost:
            if 'mysql' not in parameters:
                parameters['mysql'] = {}
            parameters['mysql']['host'] = args.mhost
        if args.mport:
            if 'mysql' not in parameters:
                parameters['mysql'] = {}
            parameters['mysql']['port'] = int(args.mport)
        if args.mdatabase:
            if 'mysql' not in parameters:
                parameters['mysql'] = {}
            parameters['mysql']['database'] = args.mdatabase
        if args.muser:
            if 'mysql' not in parameters:
                parameters['mysql'] = {}
            parameters['mysql']['user'] = args.muser
        if args.mpassword:
            if 'mysql' not in parameters:
                parameters['mysql'] = {}
            parameters['mysql']['password'] = args.mpassword
    ...
    try:
        if manager.list_tables:
            manager.list_database_tables()  # Only DAO connection, no MySQL
            ...
            # Only DAO connection needed - MySQL not opened
