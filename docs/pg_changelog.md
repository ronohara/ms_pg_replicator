...
| `-a, --no-auto-index` | Suppress automatic creation of indexes/constraints for foreign keys (skip FK if needed) |
...
- 1.40 – Simple names mode, view creation options, special character handling, foreign key fixes, mutually exclusive group correction, and connection fix for create_sane_views
